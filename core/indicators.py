"""
Local technical indicator computation (ported from hyperliquid-trading-agent).

Pure-Python, dependency-free indicator library. Accepts either:
  - a list of OHLCV candle dicts with keys: open, high, low, close, volume
  - OR a pandas DataFrame with columns: Open, High, Low, Close, Volume
    (call `df_to_candles(df)` first)

This module exposes the same surface area as the upstream hyperliquid agent's
`local_indicators.py`, so `ai_decision_maker.py` can call `compute_all()`,
`latest()`, `last_n()` against any candle source.

Adds to ProxiAlpha's existing `core/data_engine.py` indicators (which only
does RSI, MACD, SMA, BB, ATR) by also providing: ADX, OBV, VWAP, Stoch-RSI,
plus a uniform "name -> series" API so the AI decision maker's
`fetch_indicator` tool can address indicators by name.
"""
...
# ---------------------------------------------------------------------------
# DataFrame <-> candle list adapters
# ---------------------------------------------------------------------------

def df_to_candles(df: pd.DataFrame) -> list[dict]:
    """Convert a ProxiAlpha-style OHLCV DataFrame into a candle dict list.

    Accepts both capitalized (yfinance: Open/High/Low/Close/Volume) and
    ...
    for _, row in df.iterrows():
        candles.append({
            "open": float(row[col_map["open"]]) if "open" in col_map else 0.0,
            "high": float(row[col_map["high"]]) if "high" in col_map else 0.0,
            "low": float(row[col_map["low"]]) if "low" in col_map else 0.0,
            "close": float(row[col_map["close"]]) if "close" in col_map else 0.0,
...
def vwap(candles: list[dict]) -> list[float | None]:
    """Cumulative VWAP (no session reset — suitable for intraday)."""
    cum_vol = 0.0
    cum_tp_vol = 0.0
    result: list[float | None] = []
    for c in candles:
...
def compute_all(candles: list[dict]) -> dict:
    """Compute the full standard indicator suite for a candle series."""
    if not candles:
        return {}
...
def compute_all_from_df(df: pd.DataFrame) -> dict:
    """Convenience: compute the full indicator suite from a ProxiAlpha DataFrame."""
    return compute_all(df_to_candles(df))
